...
class SentinelLayoutMici(NavScroller):
  ...
  def show_event(self):
    super().show_event()
    self._status = sentineld.read_status()
    self._refresh()
  ...
  def _armed(self) -> bool:
    return ui_state.params.get_bool("SentryMode")

  def _can_arm(self) -> bool:
    return not self._armed() and not ui_state.started and not ui_state.engaged

  def _arm(self) -> None:
    ui_state.params.put_bool("SentryMode", True)
    ui_state.params.put_bool("DoReboot", True)

  def _disarm(self) -> None:
    ui_state.params.put_bool("SentryMode", False)
