...
from openpilot.selfdrive.ui.sunnypilot.mici.layouts.diagnostics import DiagnosticsLayoutMici
from openpilot.selfdrive.ui.sunnypilot.mici.layouts.sentinel import SentinelLayoutMici
...
class SettingsLayoutSP(OP.SettingsLayout):
  def __init__(self):
    ...
    diagnostics_panel = DiagnosticsLayoutMici(back_callback=gui_app.pop_widget)
    diagnostics_btn = SettingsBigButton(tr("diag"), "",
                                        gui_app.texture("icons_mici/exclamation_point.png", ICON_SIZE, ICON_SIZE))
    diagnostics_btn.set_click_callback(lambda: gui_app.push_widget(diagnostics_panel))

    sentinel_panel = SentinelLayoutMici(back_callback=gui_app.pop_widget)
    sentinel_btn = SettingsBigButton(tr("sentry"), "",
                                     gui_app.texture("icons_mici/settings/device/lkas.png", ICON_SIZE, ICON_SIZE))
    sentinel_btn.set_click_callback(lambda: gui_app.push_widget(sentinel_panel))
    ...
    items.insert(3, diagnostics_btn)
    items.insert(4, sentinel_btn)
