...
ARMED_PARAM = "SentryMode"
MIN_VOLTAGE_PARAM = "SentryMinVoltage"
...
def read_status() -> dict:
  """Lu par le panneau sentinelle. Ne fait rien de lourd."""
...
class Sentry:
  ...
  def __init__(self, params, root: str = SENTRY_DIR):
    self.params = params
    self.recorder = EventRecorder(root)
    self.motion = {cam: MotionDetector() for cam in MOTION_CAMERAS}
    self.watchdog = VoltageWatchdog(self._min_voltage())
    self.root = root
    self.voltage_mv = 0.
    self.last_frame = 0.
  ...
  def _min_voltage(self) -> int:
    try:
      return int(self.params.get(MIN_VOLTAGE_PARAM) or 11800)
    except (TypeError, ValueError):
      return 11800

  def armed(self) -> bool:
    return self.params.get_bool(ARMED_PARAM)
...
def main() -> None:
  ...
  if sentry.stop_reason in ("ignition", "voltage"):
    params.put_bool(ARMED_PARAM, False)
